...
namespace QuanLyBanHang
{
    public partial class FormNhanVien : Form
    {
        ...
        private void ExportExcel(string path)
        {
            ...
            {
                worksheet.Cells[startRow, i + 1] = dataGridViewNhanVien.Columns[i].HeaderText;
            }

            // ===== 4. Xuất dữ liệu =====
            for (int i = 0; i < dataGridViewNhanVien.Rows.Count; i++)
            {
                if (dataGridViewNhanVien.Rows[i].IsNewRow) continue;

                for (int j = 0; j < dataGridViewNhanVien.Columns.Count; j++)
                {
                    var value = dataGridViewNhanVien.Rows[i].Cells[j].Value;
            ...
            worksheet.Range[worksheet.Cells[startRow, 1], worksheet.Cells[startRow, dataGridViewNhanVien.Columns.Count]].Font.Bold = true;
            worksheet.Range[worksheet.Cells[startRow, 1], worksheet.Cells[startRow, dataGridViewNhanVien.Columns.Count]].Interior.Color =
                System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.LightGray);

            // ===== 7. Thêm border bao quanh vùng dữ liệu =====
            int lastRow = startRow + dataGridViewNhanVien.Rows.Count - 1; // tính số dòng cuối

            Excle.Range usedRange = worksheet.Range[
     worksheet.Cells[startRow, 1],
     worksheet.Cells[lastRow, dataGridViewNhanVien.Columns.Count]
 ];
